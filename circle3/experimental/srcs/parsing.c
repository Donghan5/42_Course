...
/*   Updated: 2024/10/17 22:53:42 by pzinurov         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	priority_manager(char ***tokens, int *i, t_glob_pipe *t)
{
	static int	priority;
...
int	parse(t_glob_pipe **glob_pipe, char ***tokens)
{
	...
	*glob_pipe = new_glob_pipe(NULL);
	...
	while (tokens[i])
	{
		...
		// priority_manager(tokens, &i, NULL);
				...
				&& !is_redirect(tokens[i]) && !is_paren(tokens[i])))
		{
			...
			if (is_paren(tokens[i]) == PAREN_OPEN)
				priority++;
			temp_glob->priority = priority;
			if (is_paren(tokens[i]) == PAREN_CLOSE)
				priority--;
			// priority_manager(tokens, &i, temp_glob);
			if (!tokens[i + 1])
				break ;
			i++;
		}
